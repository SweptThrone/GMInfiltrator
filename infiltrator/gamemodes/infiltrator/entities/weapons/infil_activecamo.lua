...
local CamoActivate = Sound("npc/scanner/scanner_nearmiss2.wav")
local CamoDeactivate = Sound("npc/combine_gunship/attack_stop2.wav")

local CamoMat = Material("Models/effects/comball_sphere")
...
function SWEP:PrimaryAttack()
	...
	if IsFirstTimePredicted() then
		if CamoIsOn then
			self:EmitSound( CamoDeactivate )
		else
			self:EmitSound( CamoActivate )
		end
...
function SWEP:Holster()
	if IsFirstTimePredicted() and self:GetOwner():GetNWBool( "CamoEnabled", false ) then
		self:GetOwner():SetNWBool("CamoEnabled",false)
